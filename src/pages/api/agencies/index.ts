...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createAgency() {
    ...
    const body = { ...req.body };
    if (body?.educational_content?.length > 0) {
      const create_educational_content = body.educational_content;
      body.educational_content = {
        create: create_educational_content,
      };
    } else {
      delete body.educational_content;
    }
