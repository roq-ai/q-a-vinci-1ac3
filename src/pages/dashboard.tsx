...
import { getVirtualNumbers } from 'apiSdk/virtual-numbers';
import { VirtualNumberInterface } from 'interfaces/virtual-number';
import { getSocialAccounts } from 'apiSdk/social-accounts';
import { SocialAccountInterface } from 'interfaces/social-account';

import { VirtualNumberListPage as ShowCaseList } from 'pages/virtual-numbers';
...
function DashboardPage() {
  ...
  const virtualNumberStat = useSWR<PaginatedInterface<VirtualNumberInterface>>(
    () => `/virtual-numbers`,
    () => getVirtualNumbers({ limit: 1, offset: 0 }),
  );

  const socialAccountStat = useSWR<PaginatedInterface<SocialAccountInterface>>(
    () => `/social-accounts`,
    () => getSocialAccounts({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: virtualNumberStat?.data?.totalCount,
      isLoading: virtualNumberStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Social Accounts',
      result: socialAccountStat?.data?.totalCount,
      isLoading: socialAccountStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Virtual Numbers
                </Text>
                <NextLink href={`virtual-numbers`} passHref legacyBehavior>
                  <Button
